...
>(({ className, children, showCloseButton = false, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-[50%] top-[50%] z-30 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] border bg-background px-0 py-0 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-lg",
        className
      )}
      {...props}
    >
...
const DialogHeader = ({
  ...
  <div
    className={cn(
      "flex flex-col text-center sm:text-left bg-background z-30 h-16 rounded-t-md",
      className
    )}
    {...props}
  />
...
>(
  (
    { className, showCloseButton = true, showSeparator = true, ...props },
    ref
  ) => (
    <>
      <DialogPrimitive.Title
        ref={ref}
        className={cn(
          "text-lg font-semibold leading-none tracking-tight p-6",
          className
        )}
        {...props}
      />
