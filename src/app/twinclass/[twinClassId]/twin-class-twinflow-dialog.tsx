import {TwinClassField, TwinClassFieldCreateRq, TwinFlowCreateRq} from "@/lib/api/api-types";
...
export default function CreateTwinflowDialog({
    ...
    async function onSubmit(formValues: z.infer<typeof twinflowSchema>) {
        ...
        try {
            const {data: response, error} = await api.twinflow.create({twinClassId, body: requestBody})
            if (error) {
