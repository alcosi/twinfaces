import {TwinFlowTransition, TwinFlowTransitionValidator} from "@/lib/api/api-types";
...
export function TwinflowTransitionValidators({transition, onChange}: {
    transition: TwinFlowTransition,
    onChange: () => any
}) {
    const tableRef = useRef<DataTableHandle>(null);

    const [createEditValidatorDialogOpen, setCreateEditValidatorDialogOpen] = useState<boolean>(false);
    const [editedValidator, setEditedValidator] = useState<TwinFlowTransitionValidator | null>(null);
    ...
    function getValidators() {
        return Promise.resolve({data: Object.values(transition.validators ?? {}), pageCount: 0});
    }
