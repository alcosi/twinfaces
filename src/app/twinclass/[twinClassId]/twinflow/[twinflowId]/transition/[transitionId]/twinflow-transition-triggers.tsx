import {TwinFlowTransition, TwinFlowTransitionTrigger} from "@/lib/api/api-types";
...
} from "./twinflow-transition-trigger-dialog";

export function TwinflowTransitionTriggers({transition, onChange}: {
    transition: TwinFlowTransition,
    onChange: () => any
}) {
    const tableRef = useRef<DataTableHandle>(null);

    const [createEditTriggerDialogOpen, setCreateEditTriggerDialogOpen] = useState<boolean>(false);
    const [editedTrigger, setEditedTrigger] = useState<TwinFlowTransitionTrigger | null>(null);
    ...
    function getTriggers() {
        return Promise.resolve({data: Object.values(transition.triggers ?? {}), pageCount: 0});
    }
    ...
    return <>
        ...
        <TwinflowTransitionTriggerDialog open={createEditTriggerDialogOpen} onOpenChange={setCreateEditTriggerDialogOpen}
                                         transitionId={transition.id!}
                                         trigger={editedTrigger} onSuccess={onCreateEditSuccess}/>
    </>
}