import {TwinFlowTransitionValidator, TwinFlowTransitionValidatorUpdate,} from "@/lib/api/api-types";
...
export function TwinflowTransitionValidatorDialog({
    ...
    async function createUpdateValidator(newValidator: TwinFlowTransitionValidatorUpdate) {
        try {
            ...
            if (result.error) {
                ...
                throw new Error(`Failed to ${validator ? 'update' : 'create'} validator`);
            }
            onSuccess?.();
        } catch (e) {
            console.error(e);
    ...
    return <AutoDialog
        settings={validatorAutoDialogSettings}
        {...props}
    />
}
