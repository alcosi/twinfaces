...
const SCHEMA = z.object({
  originalFactoryTriggerId: z.string().uuid(),
  newTwinFactoryId: z.string().optional().or(FIRST_ID_EXTRACTOR),
});

type FormValues = z.infer<typeof SCHEMA>;

export type FactoryTriggerDuplicateDialogRef = {
  open: (item: FactoryTrigger_DETAILED) => void;
};
...
export const FactoryTriggerDuplicateDialog = forwardRef(Component);

function Component(
  { onSuccess }: Props,
  ref: ForwardedRef<FactoryTriggerDuplicateDialogRef>
) {
  ...
  useImperativeHandle(ref, () => ({
    open: (item: FactoryTrigger_DETAILED) => {
      setSelectedName(
        item.description ?? `Trigger of factory: ${item.factory?.key}`
      );
      form.reset({
        originalFactoryTriggerId: item.id!,
        newTwinFactoryId: "",
      });
      setOpen(true);
    },
  ...
  async function handleOnSubmit(values: FormValues) {
    try {
      const { error } = await api.factoryTrigger.duplicate({
        body: {
          duplicates: [
            {
              originalFactoryTriggerId: values.originalFactoryTriggerId,
      ...
      if (error) {
        toast.error("Failed to duplicate factory trigger");
        throw error;
      }
      toast.success("Factory trigger duplicated successfully!");
      closeDialog();
      onSuccess?.();
    } catch (error) {
      console.error("Duplicate factory trigger error:", error);
      throw error;
    }
  }
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-h-[100%] sm:max-h-[80%] sm:max-w-md">
        <DialogHeader className="h-auto py-4">
          <DialogTitle>Duplicate Factory Trigger</DialogTitle>
          <DialogDescription className="pt-1">
            {selectedName
              ? `Create a duplicate for "${selectedName}".`
              : "Create a duplicate factory trigger"}
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleOnSubmit)}>
            <input
              type="hidden"
              {...form.register("originalFactoryTriggerId")}
            />
